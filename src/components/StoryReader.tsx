...
import { ArrowLeft, BookOpen, Check, ChevronDown, Loader2, Lock, ScrollText, Shield, Users } from 'lucide-react';
...
export default function StoryReader({ campaign, characters, activeCharacter, currentUserId, onUpdate, onRemoteUpdate, onCharacterUpdate, onLeave }: StoryReaderProps) {
  ...
  const availableChoices = useMemo(() => scene.choices.map(choice => ({ choice, availability: isChoiceAvailable(choice, activeCharacter, campaign.state.flags, campaign.state.inventory) })), [activeCharacter, campaign.state.flags, campaign.state.inventory, scene.choices]);

  async function selectChoice(choice: StoryChoice) {
    const availability = isChoiceAvailable(choice, activeCharacter, campaign.state.flags, campaign.state.inventory);
  ...
  async function continueStory(choice: StoryChoice, resolution: ChoiceResolution) {
      ...
      for (const item of resolution.gainedItems) if (!nextInventory.includes(item)) nextInventory.push(item);
      const nextState = {
        ...campaign.state,
      ...
      const nextScene = await generateNextScene({ bible: campaign.bible, previousScene: scene, resolution, state: nextState, characters });
      const updated: CampaignRuntime = { ...campaign, state: { ...nextState, currentSceneId: nextScene.id, currentActId: nextScene.actId }, currentScene: nextScene };
  ...
  async function finishBattle(result: BattleResult) {
    ...
    for (const consumed of result.itemsConsumed) {
      const index = equipment.indexOf(consumed);
      if (index >= 0) equipment.splice(index, 1);
    }
    for (const item of result.itemsGained) if (!equipment.includes(item)) equipment.push(item);
    const updatedCharacter: Character = { ...activeCharacter, hp_current: result.finalHp, xp: activeCharacter.xp + result.xpGained, equipment };
