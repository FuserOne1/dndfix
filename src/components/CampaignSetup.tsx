...
export default function CampaignSetup({ mode, character, onBack, onStart, loading = false, error = '', sessionCode }: CampaignSetupProps) {
  ...
  return <div className="min-h-screen bg-[#09090b] text-zinc-100 p-4 sm:p-8"><div className="max-w-5xl mx-auto space-y-6">
    ...
    <div className="grid lg:grid-cols-[1fr_280px] gap-5">
      ...
      <aside className="space-y-4"><div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Режим</p><div className="flex items-center gap-2 mt-2 font-bold">{mode === 'solo' ? <BookOpen className="text-amber-400"/> : <Users className="text-amber-400"/>}{mode === 'solo' ? 'Одиночная игра' : 'Совместная игра'}</div></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Главный герой</p><strong className="block mt-2">{character.name}</strong><span className="text-xs text-zinc-500">{character.race} · {character.class}</span></div><div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Визуальный режим</p><div className="flex gap-2 mt-2 text-sm"><Images className="w-5 h-5 text-amber-400 shrink-0"/><span>{preferences.illustrationMode === 'important' ? 'Важные сцены' : preferences.illustrationMode === 'all' ? 'Каждая сцена' : 'Только вручную'}</span></div><p className="text-[11px] text-zinc-600 mt-2">Nano Banana · примерно $0.04 за изображение</p></div>{mode === 'party' && sessionCode && <div className="rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><div className="flex justify-between"><p className="text-xs text-zinc-500 uppercase">Партия</p><span className="text-xs text-zinc-600">{participants.length}</span></div><div className="mt-2 space-y-2">{participants.map((participant,index) => <div key={index} className="flex justify-between text-xs"><span>{participant.character_snapshot.name}</span><span className={participant.is_host || participant.is_ready ? 'text-emerald-400' : 'text-zinc-600'}>{participant.is_host || participant.is_ready ? 'готов' : 'ждём'}</span></div>)}</div></div>}<button disabled={loading || preferences.tones.length === 0 || !allReady} onClick={() => void onStart(preferences)} className="w-full p-4 rounded-2xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black flex items-center justify-center gap-2 disabled:opacity-50">{loading ? <Loader2 className="animate-spin"/> : <Swords/>}{loading ? preparingPartyRoom ? 'Создаём комнату…' : 'Claude пишет кампанию…' : !allReady ? 'Ждём готовности' : preparingPartyRoom ? 'Создать комнату' : 'Начать приключение'}</button><p className="text-[11px] text-zinc-600 text-center">{preparingPartyRoom ? 'Сначала появится код приглашения. Сюжет не начнёт создаваться, пока ведущий не соберёт игроков и не запустит кампанию.' : 'Claude одновременно создаст сюжетный каркас и первую сцену. Обычно это занимает 30–100 секунд. Если API недоступен, приложение покажет ошибку, а не повторит демо-сюжет.'}</p></aside>
    </div>
  </div></div>;
}
