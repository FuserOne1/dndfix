...
import { ArrowLeft, BookOpen, Images, Loader2, Swords, Users } from 'lucide-react';
...
export default function CampaignSetup({ mode, character, onBack, onStart, loading = false, error = '', sessionCode }: CampaignSetupProps) {
  ...
  const [preferences, setPreferences] = useState<CampaignPreferences>({
    mode, length: 'short', tones: ['dark', 'mystery'], setting: '', premise: '', combatFrequency: 'balanced', difficulty: 'normal', branching: 'balanced', themes: '', boundaries: '', customWish: '', hideLockedChoices: false, voteTimerSeconds: null, illustrationMode: 'important', artStyle: 'dark-comic',
  });
  ...
  return <div className="campaign-setup-shell home-shell min-h-screen text-zinc-100"><HomeAtmosphere/><div className="home-content p-4 sm:p-8"><div className="max-w-5xl mx-auto space-y-6">
    <div className="campaign-setup-header flex items-start gap-4"><button onClick={onBack} className="p-3 rounded-xl border border-zinc-800 bg-zinc-900"><ArrowLeft/></button><div><p className="text-xs uppercase tracking-[0.22em] text-amber-500">Нулевая сессия</p><h1 className="text-3xl font-black">Настройте приключение</h1><p className="text-zinc-500 mt-1">ИИ создаст сюжетный каркас вокруг героев и этих пожеланий.</p></div><span className="campaign-seal" aria-hidden="true">✦</span></div>
    ...
    <div className="campaign-setup-layout grid lg:grid-cols-[1fr_280px] gap-5">
      <div className="campaign-form space-y-5 rounded-3xl border border-zinc-800 bg-zinc-900/60 p-5 sm:p-7">
        ...
        <Field title="Иллюстрации" hint="Создаются в фоне и не задерживают чтение"><Segment value={preferences.illustrationMode} options={[["important","Важные сцены"],["off","Вручную"],["all","Каждая сцена"]]} onChange={illustrationMode => setPreferences({ ...preferences, illustrationMode: illustrationMode as CampaignPreferences['illustrationMode'] })}/></Field>
        <Field title="Стиль изображений"><Select value={preferences.artStyle} onChange={artStyle => setPreferences({ ...preferences, artStyle: artStyle as CampaignPreferences['artStyle'] })} options={[["dark-comic","Мрачный комикс"],["classic-fantasy","Классическое фэнтези"],["graphic-novel","Графический роман"],["watercolor","Акварель"],["anime","Аниме"]]}/></Field>
      </div>
      <aside className="campaign-summary space-y-4"><div className="campaign-summary-card rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Режим</p><div className="flex items-center gap-2 mt-2 font-bold">{mode === 'solo' ? <BookOpen className="text-amber-400"/> : <Users className="text-amber-400"/>}{mode === 'solo' ? 'Одиночная игра' : 'Совместная игра'}</div></div><div className="campaign-summary-card campaign-hero-card rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Главный герой</p><strong className="block mt-2">{character.name}</strong><span className="text-xs text-zinc-500">{character.race} · {character.class}</span></div><div className="campaign-summary-card rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><p className="text-xs text-zinc-500 uppercase">Визуальный режим</p><div className="flex gap-2 mt-2 text-sm"><Images className="w-5 h-5 text-amber-400 shrink-0"/><span>{preferences.illustrationMode === 'important' ? 'Важные сцены' : preferences.illustrationMode === 'all' ? 'Каждая сцена' : 'Только вручную'}</span></div><p className="text-[11px] text-zinc-600 mt-2">Nano Banana · примерно $0.04 за изображение</p></div>{mode === 'party' && sessionCode && <div className="campaign-summary-card rounded-2xl border border-zinc-800 bg-zinc-900/70 p-4"><div className="flex justify-between"><p className="text-xs text-zinc-500 uppercase">Партия</p><span className="text-xs text-zinc-600">{participants.length}</span></div><div className="mt-2 space-y-2">{participants.map((participant,index) => <div key={index} className="flex justify-between text-xs"><span>{participant.character_snapshot.name}</span><span className={participant.is_host || participant.is_ready ? 'text-emerald-400' : 'text-zinc-600'}>{participant.is_host || participant.is_ready ? 'готов' : 'ждём'}</span></div>)}</div></div>}<button disabled={loading || preferences.tones.length === 0 || !allReady} onClick={() => void onStart(preferences)} className="campaign-start w-full p-4 rounded-2xl bg-amber-500 hover:bg-amber-400 text-zinc-950 font-black flex items-center justify-center gap-2 disabled:opacity-50">{loading ? <Loader2 className="animate-spin"/> : <Swords/>}{loading ? preparingPartyRoom ? 'Создаём комнату…' : 'Claude пишет кампанию…' : !allReady ? 'Ждём готовности' : preparingPartyRoom ? 'Создать комнату' : 'Начать приключение'}</button><p className="text-[11px] text-zinc-600 text-center">{preparingPartyRoom ? 'Сначала появится код приглашения. Сюжет не начнёт создаваться, пока ведущий не соберёт игроков и не запустит кампанию.' : 'Claude одновременно создаст сюжетный каркас и первую сцену. Обычно это занимает 30–100 секунд. Если API недоступен, приложение покажет ошибку, а не повторит демо-сюжет.'}</p></aside>
    </div>
  </div></div></div>;
}
