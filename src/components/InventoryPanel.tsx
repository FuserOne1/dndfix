  ...
  return <div className="fixed inset-0 z-[90] bg-[#09090b] text-zinc-100 overflow-y-auto">
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-8 space-y-5">
      ...
      {tab === 'inventory' && <div className="grid lg:grid-cols-[300px_1fr] gap-5">
        ...
        <section><div className="flex justify-between items-end mb-3"><div><p className="text-xs uppercase text-zinc-600">Содержимое</p><h2 className="text-xl font-bold">Предметы</h2></div><span className="text-xs text-zinc-600">{inventory.items.length}</span></div>{inventory.items.length ? <div className="grid sm:grid-cols-2 gap-3">{inventory.items.map(item => <ItemCard key={item.uid} item={item} busy={busy === `equip:${item.uid}` || busy === `quick:${item.uid}` || busy === `discard:${item.uid}`} equipped={Object.values(inventory.equipped).includes(item.uid)} quick={inventory.quickSlots.includes(item.uid)} onEquip={slot => void equip(item, slot)} onQuick={() => void toggleQuick(item)} onDiscard={() => void discard(item)} onDeposit={campaignId ? () => void deposit(item) : undefined}/>)}</div> : <Empty text="Рюкзак пуст"/>}</section>
      </div>}
...
function ItemCard({ item, busy, equipped, quick, onEquip, onQuick, onDiscard, onDeposit }: { item: InventoryItem; busy: boolean; equipped: boolean; quick: boolean; onEquip: (slot: EquipmentSlot) => void; onQuick: () => void; onDiscard: () => void; onDeposit?: () => void }) {
  const definition = getItemDefinition(item);
