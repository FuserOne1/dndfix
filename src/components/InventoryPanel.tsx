...
import { ArrowLeft, Backpack, Check, Coins, Loader2, PackageOpen, RefreshCw, Shield, ShoppingBag, Trash2, Users } from 'lucide-react';
...
export default function InventoryPanel({ character, campaignId, currentUserId = 'local', location = 'перекрёстка', sceneNumber = 1, canTrade = false, initialTab = 'inventory', onSave, onClose }: InventoryPanelProps) {
  ...
  async function useConsumable(item: InventoryItem) {
    await run(`use:${item.uid}`, async () => {
      const definition = getItemDefinition(item);
      if (!definition || definition.type !== 'consumable') throw new Error('Этот предмет нельзя использовать');
      if (!definition.effect) throw new Error('У этого предмета нет эффекта');
      ...
      if (effect.heal && effect.heal > 0) {
        const healed = Math.min(effect.heal, maxHp - currentHp);
        if (healed <= 0) throw new Error('Здоровье и так полное');
        nextHp = currentHp + healed;
        noticeParts.push(`+${healed} HP`);
      }
      if (effect.tempHp && effect.tempHp > 0) {
        noticeParts.push(`+${effect.tempHp} временных HP`);
      }
      if (!effect.heal && !effect.tempHp) {
        throw new Error('Этот предмет эффективен только в бою');
      }
      const updated: Character = { ...character, hp_current: nextHp, inventory_data: removeInventoryItem(inventory, item.uid, 1) };
      await onSave(updated);
      setInventory(updated.inventory_data!);
      setNotice(`${definition.name} использован: ${noticeParts.join(', ')}`);
    });
  }

  async function discard(item: InventoryItem) {
    await run(`discard:${item.uid}`, async () => {
      if (Object.values(inventory.equipped).includes(item.uid)) throw new Error('Сначала снимите предмет');
      if (getItemDefinition(item)?.type === 'quest') throw new Error('Сюжетный предмет нельзя выбросить');
      await commit(removeInventoryItem(inventory, item.uid, item.quantity));
      setNotice(`Выброшено: ${item.name}`);
    });
  }
  ...
  return <div className="fixed inset-0 z-[90] bg-[#09090b] text-zinc-100 overflow-y-auto">
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-8 space-y-5">
      ...
      {tab === 'inventory' && <div className="grid lg:grid-cols-[300px_1fr] gap-5">
        ...
        <section><div className="flex justify-between items-end mb-3"><div><p className="text-xs uppercase text-zinc-600">Содержимое</p><h2 className="text-xl font-bold">Предметы</h2></div><span className="text-xs text-zinc-600">{inventory.items.length}</span></div>        {inventory.items.length ? <div className="grid sm:grid-cols-2 gap-3">{inventory.items.map(item => <ItemCard key={item.uid} item={item} busy={busy === `equip:${item.uid}` || busy === `quick:${item.uid}` || busy === `discard:${item.uid}` || busy === `use:${item.uid}`} equipped={Object.values(inventory.equipped).includes(item.uid)} quick={inventory.quickSlots.includes(item.uid)} onEquip={slot => void equip(item, slot)} onQuick={() => void toggleQuick(item)} onUse={() => void useConsumable(item)} onDiscard={() => void discard(item)} onDeposit={campaignId ? () => void deposit(item) : undefined}/>)}</div> : <Empty text="Рюкзак пуст"/>}</section>
      </div>}
...
function ItemCard({ item, busy, equipped, quick, onEquip, onQuick, onUse, onDiscard, onDeposit }: { item: InventoryItem; busy: boolean; equipped: boolean; quick: boolean; onEquip: (slot: EquipmentSlot) => void; onQuick: () => void; onUse: () => void; onDiscard: () => void; onDeposit?: () => void }) {
  const definition = getItemDefinition(item);
  const canUse = definition?.type === 'consumable' && (definition.effect?.heal || definition.effect?.tempHp);
  return <article className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-4"><div className="flex gap-3"><span className="w-11 h-11 rounded-xl bg-zinc-800 flex items-center justify-center text-xl shrink-0">{definition?.icon || <PackageOpen className="w-5 h-5"/>}</span><div className="min-w-0 flex-1"><div className="flex justify-between gap-2"><strong className="truncate">{item.name}</strong>{item.quantity > 1 && <span className="text-xs text-zinc-500">×{item.quantity}</span>}</div>{definition && <p className={`text-[10px] uppercase ${RARITY_CLASS[definition.rarity]}`}>{RARITY_LABEL[definition.rarity]} · {definition.type}</p>}<p className="text-xs text-zinc-500 mt-2">{item.customDescription || definition?.description || 'Предмет из старой версии героя.'}</p></div></div><div className="flex flex-wrap gap-2 mt-4">{definition?.equipSlots?.map(slot => <button key={slot} disabled={busy || equipped} onClick={() => onEquip(slot)} className="px-3 py-2 rounded-lg bg-zinc-800 text-[11px] disabled:opacity-30">{equipped ? <span className="flex gap-1"><Check className="w-3 h-3"/>Надето</span> : SLOT_LABELS[slot]}</button>)}{canUse && <button disabled={busy} onClick={onUse} className="px-3 py-2 rounded-lg bg-emerald-500/15 text-emerald-300 text-[11px] disabled:opacity-30">Использовать</button>}{definition?.type === 'consumable' && <button disabled={busy} onClick={onQuick} className={`px-3 py-2 rounded-lg text-[11px] ${quick ? 'bg-emerald-500/15 text-emerald-300' : 'bg-zinc-800'}`}>{quick ? 'В быстром слоте' : 'В быстрый слот'}</button>}{onDeposit && <button disabled={busy || equipped} onClick={onDeposit} className="px-3 py-2 rounded-lg bg-zinc-800 text-[11px] disabled:opacity-30">В сундук</button>}<button disabled={busy || equipped || definition?.type === 'quest'} onClick={onDiscard} className="px-3 py-2 rounded-lg bg-red-500/10 text-red-300 text-[11px] disabled:opacity-30"><Trash2 className="inline w-3 h-3 mr-1"/>Выбросить</button>{busy && <Loader2 className="w-4 h-4 animate-spin text-amber-400"/>}</div></article>;
}
