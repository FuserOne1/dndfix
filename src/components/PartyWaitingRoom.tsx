...
export default function PartyWaitingRoom({ campaignId, currentUserId, onBack, onStartCampaign, starting = false, externalError = '', onCampaignStarted }: PartyWaitingRoomProps) {
  ...
  useEffect(() => {
    void refresh();
    const channel = supabase.channel(`waiting:${campaignId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'campaign_participants', filter: `campaign_id=eq.${campaignId}` }, () => void refresh()).on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'campaigns', filter: `id=eq.${campaignId}` }, payload => { if (payload.new.status === 'playing') onCampaignStartedRef.current(); }).subscribe();
  ...
  async function toggleReady() {
    ...
    else setParticipants(previous => previous.map(participant => participant.user_session_id === currentUserId ? data as ParticipantRow : participant));
  ...
  return <div className="min-h-screen bg-[#09090b] text-zinc-100 p-4 sm:p-8"><div className="max-w-3xl mx-auto space-y-6"><div className="flex gap-4"><button onClick={onBack} className="p-3 rounded-xl border border-zinc-800"><ArrowLeft/></button><div><p className="text-xs uppercase tracking-[0.2em] text-amber-500">Зал ожидания</p><h1 className="text-3xl font-black">Собирается партия</h1><p className="text-zinc-500 text-sm mt-1">Общайтесь лично или в Discord — здесь только готовность к игре.</p></div></div>
    ...
    {isHost ? <button disabled={!allReady || starting} onClick={onStartCampaign} className="w-full p-4 rounded-2xl bg-amber-500 text-zinc-950 font-black flex items-center justify-center gap-2 disabled:opacity-40">{starting && <Loader2 className="animate-spin"/>}{starting ? 'Создаём кампанию…' : allReady ? 'Начать кампанию' : 'Ждём готовности игроков'}</button> : <button disabled={!currentParticipant || updatingReady} onClick={() => void toggleReady()} className={`w-full p-4 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-50 ${ready ? 'bg-emerald-500 text-zinc-950' : 'bg-zinc-800 text-zinc-200'}`}>{updatingReady && <Loader2 className="w-4 h-4 animate-spin"/>}{updatingReady ? 'Сохраняем…' : ready ? '✓ Я готов' : 'Отметить готовность'}</button>}<p className="text-center text-xs text-zinc-600">Сюжет начнёт генерироваться только после нажатия ведущим кнопки «Начать кампанию».</p>
  </div></div>;
}
