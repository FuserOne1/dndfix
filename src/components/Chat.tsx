...
⚠️ КРИТИЧЕСКОЕ ПРАВИЛО: ЛЮБОЙ бой — даже с одним гоблином — запускается только через блок [BATTLE_START:{...}].
Если ты описываешь боевую сцену без этого блока — мини-игра НЕ запустится и кнопка НЕ появится.

ОБЯЗАТЕЛЬНО при начале любого боевого столкновения добавь [BATTLE_START:{...}] в конце сообщения.
...
НИКОГДА не описывай бой текстом (броски, попадания, урон) — это делает мини-игра. Твоя задача: описать атмосферу, врагов и окружение, а затем вставить блок с данными.

После отправки блока [BATTLE_START] игрок увидит кнопку "Начать бой" в чате, откроется мини-игра. ВСЯ механика боя обрабатывается кодом (кубы, урон, порядок ходов, AI врагов). ИИ НЕ участвует в бою (экономия токенов). После окончания боя результат (XP, лут, HP) автоматически применяется к персонажу.
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center p-4">
        <div className="text-center space-y-4 max-w-md">
          ...
          <p className="text-red-500 text-xs">sessionId: {sessionId}</p>
        </div>
      </div>
    );
  }
...
`).join('\n');
      ...
      for (const jsonStr of allStatsUpdateJSONs) {
        ...
        try {
          ...
          if (prevStats) {
            ...
            const lostItems = prevEquipment.filter(item => !newEquipment.includes(item));

            if (gainedItems.length > 0) {
              gainedItems.forEach(item => {
                changes.push(`🎒 Получено: ${item}`);
              });
            }

            if (lostItems.length > 0) {
              lostItems.forEach(item => {
                changes.push(`🗑️ Потеряно/Использовано: ${item}`);
              });
            }

            if (changes.length > 0) {
              allChanges.push(`**${playerName}**:\n` + changes.join('\n'));
            }
          }
          ...
          // Обновляем базу данных только если есть изменения
          if (hasChanges) {
            console.log('📊 Stats changed, updating DB');
            updateRoomStats(playerName, updatedStats);
          } else {
            console.log('📊 Stats unchanged, skipping DB update');
          }
      ...
      // Формируем итоговое уведомление со всеми изменениями
      ...
      // ═══════════════════════════════════════════════════════════
      // ПАРСИНГ НОВОГО БОЕВОГО БЛОКА (мини-игра) — ДО стрипа!
      // ═══════════════════════════════════════════════════════════
      console.log('🔍 aiText contains BATTLE_START?', aiText.includes('[BATTLE_START:'));
        ...
        <div className="flex items-center gap-1 md:gap-4 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1 md:gap-2 flex-wrap">
              <p className="text-[9px] md:text-xs text-zinc-500 font-mono uppercase tracking-widest truncate">Комната: {sessionId}</p>
              <button
