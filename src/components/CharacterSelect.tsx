...
export default function CharacterSelect({ userSessionId, onCharacterSelected, onBack, roomId }: CharacterSelectProps) {
  ...
  const handleCreateCharacter = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Формируем story_summary с чертой и особым предметом
      let storySummary = '';
      ...
      if (newCharacter.specialItem.trim()) {
        ...
        if (newCharacter.specialItemDescription.trim()) {
          storySummary += ' (' + newCharacter.specialItemDescription.trim() + ')';
        }
      }
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              <div className="sticky top-0 bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex items-center justify-between">
                ...
                <button onClick={() => { setShowCreateForm(false); setNewCharacter({ name: '', race: 'Человек', class: 'Воин', background: '', feat: '', specialItem: '', specialItemDescription: '', customEquipment: '', gold: 0, abilities: '', avatar_icon: 'warrior' }); setPointBuy({ strength: 8, dexterity: 8, constitution: 8, intelligence: 8, wisdom: 8, charisma: 8 }); }} className="text-zinc-400 hover:text-white"><span className="text-2xl">×</span></button>
              </div>
              <form onSubmit={handleCreateCharacter} className="p-6 space-y-6">
                <div className="space-y-4">
                  ...
                  <div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Способности</label><textarea value={newCharacter.abilities} onChange={(e) => setNewCharacter({ ...newCharacter, abilities: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" rows={3} placeholder="Ярость (2 раза в день), Заговоры: Свет, Дружба с животными, Заклинания 1 уровня: Лечение ран, Благословение..." /></div>
                  ...
                  <div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Деньги (золото)</label><input type="number" value={newCharacter.gold} onChange={(e) => setNewCharacter({ ...newCharacter, gold: parseInt(e.target.value) || 0 })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="0" min="0" /></div>
