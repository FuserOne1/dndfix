...
export default function CharacterSelect({ userSessionId, onCharacterSelected, onBack, roomId }: CharacterSelectProps) {
  ...
  return (
    <div className={`min-h-screen bg-zinc-950 text-zinc-100 flex flex-col font-sans ${theme}`}>
      ...
      {/* Форма создания */}
      <AnimatePresence>{showCreateForm && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4"><div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"><div className="sticky top-0 bg-zinc-900 border-b border-zinc-800 px-6 py-4 flex items-center justify-between"><h2 className="text-lg font-bold text-white">Создание персонажа</h2><button onClick={() => { setShowCreateForm(false); setNewCharacter({ name: '', race: 'Человек', class: 'Воин', background: '', specialItem: '', specialItemDescription: '' }); setPointBuy({ strength: 8, dexterity: 8, constitution: 8, intelligence: 8, wisdom: 8, charisma: 8 }); }} className="text-zinc-400 hover:text-white"><span className="text-2xl">×</span></button></div><form onSubmit={handleCreateCharacter} className="p-6 space-y-6"><div className="space-y-4"><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Имя</label><input value={newCharacter.name} onChange={(e) => setNewCharacter({ ...newCharacter, name: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Арагорн" required /></div><div className="grid grid-cols-2 gap-4"><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Раса</label><select value={newCharacter.race} onChange={(e) => setNewCharacter({ ...newCharacter, race: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50">{races.map((r) => (<option key={r.name} value={r.name}>{r.name}</option>))}</select></div><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Класс</label><select value={newCharacter.class} onChange={(e) => setNewCharacter({ ...newCharacter, class: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50">{classes.map((c) => (<option key={c.name} value={c.name}>{c.name}</option>))}</select></div></div><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Предыстория</label><textarea value={newCharacter.background} onChange={(e) => setNewCharacter({ ...newCharacter, background: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" rows={3} placeholder="Искатель приключений..." /></div><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Особый предмет</label><input value={newCharacter.specialItem} onChange={(e) => setNewCharacter({ ...newCharacter, specialItem: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" placeholder="Меч предков" /></div><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-2">Описание особого предмета</label><textarea value={newCharacter.specialItemDescription} onChange={(e) => setNewCharacter({ ...newCharacter, specialItemDescription: e.target.value })} className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-primary/50" rows={2} placeholder="Древний меч, передаваемый в семье..." /></div><div><label className="block text-xs font-bold uppercase tracking-widest text-zinc-500 mb-3">Аватар</label><div className="flex gap-3">{avatarOptions.map((avatar) => (<button key={avatar.id} type="button" onClick={() => setNewCharacter({ ...newCharacter, avatar_icon: avatar.id })} className={`p-4 rounded-xl border-2 transition-all ${newCharacter.avatar_icon === avatar.id ? 'border-primary bg-primary/10' : 'border-zinc-800 bg-zinc-950 hover:border-zinc-700'}`} title={avatar.label}><span className="text-3xl block">{avatar.emoji}</span></button>))}</div></div><div><div className="flex items-center justify-between mb-3"><label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Характеристики (Point Buy)</label><span className={`text-xs font-bold ${pointsRemaining === 0 ? 'text-green-500' : 'text-amber-500'}`}>Осталось очков: {pointsRemaining}</span></div><div className="grid grid-cols-3 gap-3">{Object.entries(pointBuy).map(([stat, value]) => (<div key={stat} className="p-3 bg-zinc-950 border border-zinc-800 rounded-xl"><div className="flex items-center justify-between mb-2"><span className="text-xs text-zinc-500 uppercase">{stat === 'strength' ? 'Сила' : stat === 'dexterity' ? 'Ловкость' : stat === 'constitution' ? 'Телосложение' : stat === 'intelligence' ? 'Интеллект' : stat === 'wisdom' ? 'Мудрость' : 'Харизма'}</span><span className="text-sm font-bold text-white">{value}</span></div><div className="flex items-center gap-2"><button type="button" onClick={() => adjustStat(stat as keyof typeof pointBuy, -1)} disabled={value <= 8} className="w-8 h-8 rounded-lg bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-white font-bold">-</button><div className="flex-1" /><button type="button" onClick={() => adjustStat(stat as keyof typeof pointBuy, 1)} disabled={value >= 15 || pointsRemaining <= 0} className="w-8 h-8 rounded-lg bg-zinc-800 hover:bg-zinc-700 disabled:opacity-50 text-white font-bold">+</button></div></div>))}</div></div></div><button type="submit" disabled={isJoining || pointsRemaining !== 0} className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/80 text-white rounded-2xl py-4 font-bold uppercase tracking-widest transition-all disabled:opacity-50">{isJoining ? <Loader2 className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />}{pointsRemaining !== 0 ? `Распределите все очки (осталось: ${pointsRemaining})` : 'Создать персонажа'}</button></form></div></motion.div>)}
    </div>
  );
}
