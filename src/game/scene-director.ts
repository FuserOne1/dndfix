...
export function buildScenePlan(bible: CampaignBible, preferences: CampaignPreferences, characters: Character[]): SceneBlueprint[] {
  ...
  for (let index = 0; index < total; index += 1) {
    ...
    const purpose = seed || (type === 'climax' ? act.turningPoint : type === 'ending' ? bible.centralConflict : act.goal);
  ...
  if (characters.length) ensurePersonalCoverage(plan, characters);
...
function ensurePersonalCoverage(plan: SceneBlueprint[], characters: Character[]) {
  const usable = plan.map((_, index) => index).filter(index => index > 1 && index < plan.length - 2 && !['combat', 'trade', 'rest'].includes(plan[index].type));
  characters.forEach((character, characterIndex) => {
    ...
    plan[index] = {
      ...
      focusCharacter: character.name, purpose: `Продвинуть личную линию героя ${character.name}`,
    };
  });
}
